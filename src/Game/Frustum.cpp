...
{
	m_planes[static_cast<size_t>(FrustumPlanes::Right)]  = { 0.0f };
	m_planes[static_cast<size_t>(FrustumPlanes::Left)]   = { 0.0f };
	m_planes[static_cast<size_t>(FrustumPlanes::Top)]    = { 0.0f };
	m_planes[static_cast<size_t>(FrustumPlanes::Bottom)] = { 0.0f };
	m_planes[static_cast<size_t>(FrustumPlanes::Front)]  = { 0.0f };
	m_planes[static_cast<size_t>(FrustumPlanes::Back)]   = { 0.0f };
}
//-----------------------------------------------------------------------------
void Frustum::Extract() noexcept
{
	...
	m_planes[static_cast<size_t>(FrustumPlanes::Right)] = { planes.m3 - planes.m0, planes.m7 - planes.m4, planes.m11 - planes.m8, planes.m15 - planes.m12 };
	normalizePlane(m_planes[static_cast<size_t>(FrustumPlanes::Right)]);

	m_planes[static_cast<size_t>(FrustumPlanes::Left)] = { planes.m3 + planes.m0, planes.m7 + planes.m4, planes.m11 + planes.m8, planes.m15 + planes.m12 };
	normalizePlane(m_planes[static_cast<size_t>(FrustumPlanes::Left)]);

	m_planes[static_cast<size_t>(FrustumPlanes::Top)] = { planes.m3 - planes.m1, planes.m7 - planes.m5, planes.m11 - planes.m9, planes.m15 - planes.m13 };
	normalizePlane(m_planes[static_cast<size_t>(FrustumPlanes::Top)]);

	m_planes[static_cast<size_t>(FrustumPlanes::Bottom)] = { planes.m3 + planes.m1, planes.m7 + planes.m5, planes.m11 + planes.m9, planes.m15 + planes.m13 };
	normalizePlane(m_planes[static_cast<size_t>(FrustumPlanes::Bottom)]);

	m_planes[static_cast<size_t>(FrustumPlanes::Back)] = { planes.m3 - planes.m2, planes.m7 - planes.m6, planes.m11 - planes.m10, planes.m15 - planes.m14 };
	normalizePlane(m_planes[static_cast<size_t>(FrustumPlanes::Back)]);

	m_planes[static_cast<size_t>(FrustumPlanes::Front)] = { planes.m3 + planes.m2, planes.m7 + planes.m6, planes.m11 + planes.m10, planes.m15 + planes.m14 };
	normalizePlane(m_planes[static_cast<size_t>(FrustumPlanes::Front)]);
}
//-----------------------------------------------------------------------------
...
bool Frustum::PointIn(const Vector3& position) const noexcept
{
	...
	{
		if (DistanceToPlane(m_planes[i], position) <= 0) // point is behind plane
			return false;
	}
...
bool Frustum::PointIn(float x, float y, float z) const noexcept
{
	for (size_t i = 0; i < MaxFrustumPlanes; i++)
	{
		if (DistanceToPlane(m_planes[i], x, y, z) <= 0) // point is behind plane
			return false;
	}
...
bool Frustum::SphereIn(const Vector3& position, float radius) const noexcept
{
	for (size_t i = 0; i < MaxFrustumPlanes; i++)
	{
		if (DistanceToPlane(m_planes[i], position) < -radius) // center is behind plane by more than the radius
			return false;
	}
...
bool Frustum::AABBoxIn(const Vector3& min, const Vector3& max) const noexcept
{
	...
	for (size_t i = 0; i < MaxFrustumPlanes; i++)
	{
		...
		if (DistanceToPlane(m_planes[i], min.x, min.y, min.z) >= 0)
			oneInside = true;

		if (DistanceToPlane(m_planes[i], max.x, min.y, min.z) >= 0)
			oneInside = true;

		if (DistanceToPlane(m_planes[i], max.x, max.y, min.z) >= 0)
			oneInside = true;

		if (DistanceToPlane(m_planes[i], min.x, max.y, min.z) >= 0)
			oneInside = true;

		if (DistanceToPlane(m_planes[i], min.x, min.y, max.z) >= 0)
			oneInside = true;

		if (DistanceToPlane(m_planes[i], max.x, min.y, max.z) >= 0)
			oneInside = true;

		if (DistanceToPlane(m_planes[i], max.x, max.y, max.z) >= 0)
			oneInside = true;

		if (DistanceToPlane(m_planes[i], min.x, max.y, max.z) >= 0)
			oneInside = true;
