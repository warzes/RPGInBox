...
CanoeSprite::CanoeSprite(std::shared_ptr<Texture> bmp)
...
ShipSprite::ShipSprite(std::shared_ptr<Texture> bmp)
    : VehicleSprite(bmp, 0)
{
    VehicleSprite::Start();
}
...
VehicleSprite::VehicleSprite(std::shared_ptr<Texture> bmp, int frameOffsetY)
    : sprite(bmp),
    moving(false),
    direction(Dir_Down),
...
void VehicleSprite::DrawAt(float screenX, float screenY)
{
...
AirshipSprite::AirshipSprite(std::shared_ptr<Texture> bmp)
    : sprite(bmp),
    direction(Dir_Down),
    bitmap(bmp),
    x(0),
...
void AirshipSprite::DrawAt(float screenX, float screenY)
{
    DrawTexturePro(*bitmap,
        { 0.0f, 12 * 16, 16, 16 },
